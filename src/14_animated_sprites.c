...
int get_animation_state(struct timespec* start_time, int* index);
...
int init_SDL(struct sdl_system* system) {
    ...
    system->window = SDL_CreateWindow(
        "SDL Tutorial 14 - Animated Sprites and VSync", SDL_WINDOWPOS_UNDEFINED,
...
int get_animation_state(struct timespec* start_time, int* index) {
    ...
    if (seconds < (duration / 2)) {
        // sprites from index 0 to 12
        double t = seconds / (duration / 2);
        *index = (int)lround((1.0 - t) * 0 + t * 12.0);
    } else {
        // sprites from index 12 to 0
        double t = (seconds - ((duration / 2))) / (duration / 2);
