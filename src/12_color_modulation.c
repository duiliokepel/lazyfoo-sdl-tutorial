...
int init_SDL(struct sdl_system* system) {
    ...
    system->window = SDL_CreateWindow(
        "SDL Tutorial 12 - Color Modulation", SDL_WINDOWPOS_UNDEFINED,
...
int load_media(struct sdl_data* data, SDL_Renderer* renderer) {
    ...
    return_code = load_texture_embedded(
        &(data->color_modulation), _embed_color_modulation_png_start,
...
int handle_events(bool* quit, uint8_t* red, uint8_t* green, uint8_t* blue) {
    ...
    ASSERT (red != NULL, "Argument red must not be NULL") { return -1; }
    ASSERT (green != NULL, "Argument green must not be NULL") { return -1; }
    ASSERT (blue != NULL, "Argument blue must not be NULL") { return -1; }
...
int main_loop(const struct sdl_system system, const struct sdl_data data) {
    ...
    while (quit == false) {
        ...
        return_code = SDL_SetTextureColorMod(data.color_modulation.texture, red,
                                             green, blue);
        ASSERT (return_code == 0, "SDL_SetTextureColorMod error=[%s]",
                SDL_GetError()) {
            return -1;
        }
        ...
        // Render texture
        return_code =
            render_texture(data.color_modulation, system.renderer, 0, 0, NULL);
