...
int set_trace_file(const char *path) {
    ...
    if (length >= sizeof(trace_path)) {
        fprintf(stderr, "%s:%d - %s - length of path exceeds PATH_MAX=[%zu]\n",
                __FILE__, __LINE__, __func__, sizeof(trace_path));
        return -1;
    }
    ...
    if (trace_file == NULL) {
        ...
        fprintf(stderr, "%s:%d - %s - Failed to open file \"%s\" error=[%s]\n",
                __FILE__, __LINE__, __func__, trace_path, strerror(error_num));
        return -1;
    }
...
static int timestampISO8601(char *buffer, const size_t buffer_size,
    ...
    if (buffer_size < TIMESTAMP_SIZE) {
        fprintf(stderr,
                "%s:%d - %s - Argument buffer_size=[%zu] should not be smaller "
                "than TIMESTAMP_SIZE=[%d]\n",
    ...
    if (return_tm_pointer == NULL) {
        ...
        fprintf(stderr, "%s:%d - %s - localtime_r error=[%s]\n", __FILE__,
                __LINE__, __func__, strerror(error_num));
        return -1;
    }
    // YYYY-MM-DDTHH:MM:SS
    ...
    return_code = snprintf(buffer, buffer_size, "%s,%09ld%s", date,
                           (long)timestamp->tv_nsec, timezone2);
    if (return_code < 0) {
        int error_num = errno;
    ...
    if ((size_t)return_code >= buffer_size) {
        fprintf(stderr,
                "%s:%d - %s - return_code=[%i] should not be bigger than "
                "buffer_size=[%zu]\n",
                __FILE__, __LINE__, __func__, return_code, buffer_size);
        memset(buffer, 0, buffer_size);
        return -1;
    }
...
int _trace_va(const char *file, int line, const char *function,
    ...
    return_code =
        timestampISO8601(time_string, sizeof(time_string), &timestamp);
    if (return_code < 0) {
    ...
    if (trace_file != NULL) {
        ...
        return_code = fprintf(trace_file, "%s - %s:%d - %s - ", time_string,
                              file, line, function);
        if (return_code < 0) {
            int error_num = errno;
    ...
    // output to stdout
    {
        ...
        // output YYYY-MM-DDTHH:MM:SS,nnnnnnnnn+HH:MM file:line - function -
        return_code = fprintf(stdout, "%s - %s:%d - %s - ", time_string, file,
                              line, function);
        if (return_code < 0) {
            int error_num = errno;
            fprintf(stderr, "%s:%d - %s - fprintf error=[%s]\n", __FILE__,
