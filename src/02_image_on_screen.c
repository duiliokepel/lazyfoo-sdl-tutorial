...
int init_SDL(struct sdl_system* system) {
    ...
    system->window =
        SDL_CreateWindow("SDL Tutorial 02 - Getting an Image on the Screen",
...
int load_media(struct sdl_data* data) {
    ...
    ASSERT (data->image_hello_world == NULL,
            "Argument data->image_hello_world must be NULL before load_media") {
        return -1;
    }

    TRACE("Opening stream to embedded hello_world.bmp");
    image_RWops = SDL_RWFromConstMem(_embed_hello_world_bmp_start,
                                     (int)_embed_hello_world_bmp_size);
    ASSERT (image_RWops != NULL, "SDL_RWFromConstMem error=[%s]",
            SDL_GetError()) {
        return -1;
    }
    ...
    ASSERT (data->image_hello_world != NULL, "SDL_LoadBMP_RW error=[%s]",
            SDL_GetError()) {
        SDL_RWclose(image_RWops);
        return -1;
    }

    TRACE("Closing stream to embedded hello_world.bmp");
    return_code = SDL_RWclose(image_RWops);
    ASSERT (return_code == 0, "SDL_RWclose error=[%s]", SDL_GetError()) {
        SDL_FreeSurface(data->image_hello_world);
        data->image_hello_world = NULL;
        return -1;
    }

    TRACE("Image width=[%d] height=[%d]", data->image_hello_world->w,
          data->image_hello_world->h);
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    return_code = SDL_BlitSurface(data.image_hello_world, NULL,
                                  system.screen_surface, NULL);
    ASSERT (return_code == 0, "SDL_BlitSurface error=[%s]", SDL_GetError()) {
        free_media(&data);
        close_SDL(&system);
        return -1;
    }
